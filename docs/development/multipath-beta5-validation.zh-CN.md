...
本次使用协议 v9：session-close 帧新增受限的关闭原因字段，状态 schema 3 新增 `last_error_source`。双端都需要更新。业务侧关闭与先前已发生的 MP 故障分别记录；只有关闭类 I/O 事件继承业务端来源，来源缺失、超时及格式错误均不据此过滤。调度、流控、重注入、FIN／排空行为和关闭时机不变。

新增回归覆盖关闭原因编解码、跨路径来源传播、主路径故障归因、QUIC code-zero 取消分类，以及 LuCI 按来源过滤。正确性 race 测试（排除历史 `TestPerformanceHealthyLinks`）及 direct／代理子路径 TFO 集成矩阵通过。下文性能数据是历史结果，不是本次重新测量。
...
原 beta5 发布提交 `a6e18e4d8` 在 `cd94d40a6` 基础上仅修正统计：路径挂接次数不再依赖本侧发送聚合激活，已报告的远端失败次数纳入关闭连接的累计值。调度、路径估计、v8 线协议和数据传输逻辑均不变。新增计数测试的五轮 race、正确性 race 回归（排除 `TestPerformanceHealthyLinks`）、vet 和 direct／代理子路径 TFO 集成测试再次通过。该次发布仅修正统计，未修改或重跑下文记录的历史性能失败项。
